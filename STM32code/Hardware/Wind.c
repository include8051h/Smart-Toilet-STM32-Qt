...
{
	//打开RCC时钟
	RCC_APB2PeriphClockCmd(Wind_RCC, ENABLE);
...
//开漏输出通常用于总线（如I2C）或者需要电平转换的场合。
	...
	GPIO_InitStructure.GPIO_Pin = Wind_Pin;//选中B1号
	
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//GPIO速度一般选择50MHz
	
	GPIO_Init(Wind_Port, &GPIO_InitStructure);
...
void Wind_OFF(void)
{
	GPIO_ResetBits(Wind_Port, Wind_Pin);
}

void Wind_ON(void)
{
	GPIO_SetBits(Wind_Port, Wind_Pin);
}
